...
def ep_version_setup():
    ...
    os.environ['ENERGYPLUS_VER'] = "9.3.0"
    yield None
    if previous_set_ver:
...
def config_many_vertices():
    ...
    config["EPLUS_ADAPTER"] = {"EPLUS_VERSION": "9.3"}
...
def test_window_shading(result_main_folder, default_main_cfg, ep_version_setup, caplog):
    ...
    all_config["EPLUS_ADAPTER"]["EPLUS_VERSION"] = "9.3"
    ...
    base_folder = str(result_main_folder / Path("win_shading_ep93"))
    ...
    expected_output = os.path.dirname(__file__) / Path("expected_result") / Path("win_shading") / Path("fid_4_win_shading_ep93.idf")
...
def test_night_vent(result_main_folder, default_main_cfg, ep_version_setup, caplog):
    ...
    expected_output = os.path.dirname(__file__) / Path("expected_result") / Path("night_vent") / Path("fid_2_night_vent_ep93.idf")
    assert test_helpers.are_files_equal(idf_output, expected_output, ignore_changing_config=True, ignore_filesep_mismatch=True, ignore_line_nrs=[1])

def test_many_vertices_building(result_main_folder, config_many_vertices, ep_version_setup):
    fid = 14002
    res_base_path = result_main_folder / Path("sample_case_many_vertices_93")
