...
def test_energy_mix_co2_coeffs(ureg, nep_es_cfg):
    energy_mix = EnergyMix(ureg, nep_es_cfg)
    metadata, expected_co2_coeffs = read_csvy_raw(expected_res_folder / Path("energymix_nep_co2_coeffs.csvy"), separator=";")
    ...
    for period_name, expected_period_entries in expected_co2_coeffs.items():
        ...
        assert result_co2_coeffs.values() == pytest.approx(expected_period_entries.values.tolist(), rel=0.01)


def test_energy_mix_pen_factors(ureg, nep_es_cfg):
    energy_mix = EnergyMix(ureg, nep_es_cfg)
    metadata, expected_pen_factors = read_csvy_raw(expected_res_folder / Path("energymix_nep_pen_factors.csvy"), separator=";")
    ...
    for period_name, expected_period_entries in expected_pen_factors.items():
        ...
        assert result_pen_factors.values() == pytest.approx(expected_period_entries.values.tolist(), rel=0.01)


def test_system_efficiencies(nep_es_cfg):
    system_eff = SystemEfficiencies(nep_es_cfg)
    assert system_eff.get_dhw_system_efficiency(EnergySource.HEAT_PUMP, 2030) == 3.12
    assert system_eff.get_heating_system_efficiency(EnergySource.WOOD, 2050) == 0.82
...
def test_full_retrofit_rate(custom_conf_nep):
    ...
    assert full_ret_rates_per_ac[AgeClass(max_age=1918)] == 0.09
...
def test_partial_retrofit_rate_withebox(custom_conf_nep):
    ...
    for (bldg_elems, partial_share) in partial_ret_share_per_ac[AgeClass(min_age=1975, max_age=1984)]:
        if bldg_elems == [BuildingElement.WINDOW]:
            assert partial_share == pytest.approx(0.4583)
            break


def test_partial_retrofit_rate(custom_conf_nep):
    ret_rates = RetrofitRates(custom_conf_nep)
    ...
    assert len(partial_ret_rates) == 3
    ret_rate_sum = 0
