...
def get_sample_opaque_constr(ureg: pint.UnitRegistry):
    ...
    return Construction(name="the_sample_construction", layers=the_layers, bldg_element=BuildingElement.WALL)
...
def get_sample_win_constr(ureg: pint.UnitRegistry):
    ...
    win_glass_layer = WindowLayer(name="TestGlazing", material=get_sample_win_glazing_mat(ureg), thickness=0.006 * ureg.m)

    win_constr_name = "My_Window_Glass_Constr_Test"
    return WindowConstruction(
        frame=win_frame,
        glass=WindowGlassConstruction(
            name=win_constr_name, layers=[win_glass_layer, win_air_layer, win_glass_layer], emb_co2_emission_per_m2=None, emb_non_ren_primary_energy_per_m2=None
        ),
        shade=win_shade,
    )
...
def test_add_construction(ureg, idf, idf_res_path):
    ...
    assert are_files_equal(idf_res_path, os.path.dirname(__file__) / Path("./expected_results/constr/constr_opaque.idf"), ignore_line_nrs=[1])
...
def test_add_window_construction(ureg, idf, idf_res_path):
    ...
    assert are_files_equal(idf_res_path, os.path.dirname(__file__) / Path("./expected_results/constr/constr_window.idf"), ignore_line_nrs=[1])


def test_add_opaque_material(ureg, idf, idf_res_path):
    sample_mat = get_sample_opaque_mat(ureg)
    idf_writer_construction.add_opaque_material(idf=idf, idf_obj_name="Glasswool_TEST.2500", thickness=0.25 * ureg.m, mat_def=sample_mat, ureg=ureg)
    idf.save(idf_res_path)

    assert are_files_equal(idf_res_path, os.path.dirname(__file__) / Path("./expected_results/constr/opaque_mat.idf"), ignore_line_nrs=[1])


def test_add_win_glazing_material(ureg, idf, idf_res_path):
    sample_win_glazing_mat = get_sample_win_glazing_mat(ureg)
    idf_writer_construction.add_window_glazing_material(idf=idf, idf_obj_name="my_test_win_mat", thickness=0.006 * ureg.m, mat_def=sample_win_glazing_mat, ureg=ureg)
    idf.save(idf_res_path)
    assert are_files_equal(idf_res_path, os.path.dirname(__file__) / Path("./expected_results/constr/window_glazing_mat.idf"), ignore_line_nrs=[1])
