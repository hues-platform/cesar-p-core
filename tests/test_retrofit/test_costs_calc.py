...
def test_window_costs_not_defined():
    ureg = cesarp.common.init_unit_registry()
    retCosts = ConstructionRetrofitCosts(ureg, {})
    glass_constr = WindowGlassConstruction("http://uesl_data/sources/archetypes/windows/AFancyWindow", None, None, None)
    ret_win = WindowConstruction(glass=glass_constr, frame=None, shade=None)
...
def test_constr_costs():
    ureg = cesarp.common.init_unit_registry()
    my_constr = Construction(
        name="test_constr",
        layers=[
            Layer(name="outside", thickness=0.02 * ureg.m, material=None, function=LayerFunction.OUTSIDE_FINISH, retrofitted=True),
            Layer(name="insulation_new", thickness=0.05 * ureg.m, material=None, function=LayerFunction.INSULATION_OUTSIDE_BACK_VENTILATED, retrofitted=True),
            Layer(name="insulation_old", thickness=0.16 * ureg.m, material=None, function=LayerFunction.INSULATION_IN_BETWEEN, retrofitted=False),
        ],
    ...
    assert retCosts.get_costs_for_construction_retrofit(my_constr) == (expected_cost_layer2_19_5cm + expected_cost_layer3_16cm)
