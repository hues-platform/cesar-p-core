...
def get_mock_wall_construction_for_emission_calc(ureg: pint.UnitRegistry):
    ...
    layer1 = Layer(name="zz", retrofitted=True, thickness=0.1 * ureg.m, material=render, function=LayerFunction.OUTSIDE_FINISH)
...
def get_mock_window_construction_for_emission_calc(ureg: pint.UnitRegistry):
    ...
    frame = WindowFrameConstruction(emb_co2_emission_per_m2=256 * ureg.kg * ureg.CO2eq / ureg.m**2, emb_non_ren_primary_energy_per_m2=3740 * ureg.MJ * ureg.Oileq / ureg.m**2)
...
def bldg_shape_detailed_test_site_fid2():
    geom_fact = GeometryBuilderFactory(
        read_sitevertices_from_csv(
            os.path.dirname(__file__) / Path("./testfixture/SiteVertices_bldg_fid2.csv"), {"gis_fid": "TARGET_FID", "height": "HEIGHT", "x": "POINT_X", "y": "POINT_Y"}
        ),
...
def bldg_shape_detailed_non_rect_footprint():
    geom_fact = GeometryBuilderFactory(
        read_sitevertices_from_csv(
            os.path.dirname(__file__) / Path("./testfixture/SiteVertices_non_rectangular.csv"), {"gis_fid": "TARGET_FID", "height": "HEIGHT", "x": "POINT_X", "y": "POINT_Y"}
        ),
        {"GEOMETRY": {"NEIGHBOURHOOD": {"RADIUS": 0}}},
    )
